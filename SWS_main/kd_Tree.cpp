...
double kdTree::count_space(kdNode*node, statistics& stat)
{
	double kd_tree_space = node->idList.size() * sizeof(int) + stat.dim * 2 * sizeof(double);

	if (node->childVector.size() != 0)
	{
		for (int c = 0; c < node->childVector.size(); c++)
			kd_tree_space += count_space((kdNode*)node->childVector[c], stat);
	}

	return kd_tree_space;
}
