...
void visual_Algorithm(statistics& stat)
{
	...
	if (stat.method == 2 || stat.method == 4 || stat.method == 8) //SWS (parallel) for method = 8, SWS for method = 4, RQS (ball-tree) for method = 2
	{
		...
		if (stat.method == 4 || stat.method == 8)
			init_SWS(stat);
	}

	if (stat.method >= 0 && stat.method <= 4) //single-thread setting
	{
