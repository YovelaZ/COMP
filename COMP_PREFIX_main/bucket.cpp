...
void bucket_algorithm_row(statistics& stat, double k, int y_index, vector<double**>& stat_plane_vec)
{
	...
	for (int x_index = 0; x_index < stat.n_x; x_index++)
	{
		for (int e = 0; e < (int)stat.B_L[x_index].size(); e++)
		{
			...
			for (int u = 0; u <= 2; u++)
			{
				stat.C_L[u] += t_p_u* stat.sorted_featureVector[id][3];

				p_square_norm = 0;
				for (int d = 0; d < dim - 1; d++)
				{
					stat.v_L[u][d] += t_p_u * stat.sorted_featureVector[id][d]* stat.sorted_featureVector[id][3];
					p_square_norm += stat.sorted_featureVector[id][d] * stat.sorted_featureVector[id][d]* stat.sorted_featureVector[id][3];
				}
		...
		for (int e = 0; e < (int)stat.B_U[x_index].size(); e++)
		{
			...
			for (int u = 0; u <= 2; u++)
			{
				stat.C_U[u] += t_p_u* stat.sorted_featureVector[id][3];

				p_square_norm = 0;
				for (int d = 0; d < dim - 1; d++)
				{
					stat.v_U[u][d] += t_p_u * stat.sorted_featureVector[id][d]* stat.sorted_featureVector[id][3];
					p_square_norm += stat.sorted_featureVector[id][d] * stat.sorted_featureVector[id][d]* stat.sorted_featureVector[id][3];
				}
