...
double ballTree::count_space(ballNode*node, statistics& stat)
{
	double ball_tree_space = 0;
	
	ball_tree_space += node->idList.size() * sizeof(int) + (stat.dim + 2) * sizeof(double);

	for (int c = 0; c < (int)node->childVector.size(); c++)
		ball_tree_space += count_space((ballNode*)node->childVector[c], stat);

	return ball_tree_space;
}