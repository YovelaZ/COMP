...
void initStat(int argc, char**argv, statistics& stat)
{
	...
	stat.runtime = 0;
	//debug
	/*stat.dataFileName = (char*)"../../../Datasets/Atlanta/Atlanta";
	stat.outputFileName = (char*)"./Results/Atlanta_X32_Y32_Z32_M7_gs_1_gt_1";
	stat.method = 7;
	stat.n_x = 32;
	stat.n_y = 32;
	stat.n_t = 32;
	...
	stat.b = 10;
	stat.specify_region = 0;
	stat.is_progressive = 0;*/
...
//    file >> temp_string;
    ...
    stat.gamma_s = stat.gamma_ratio_s / h_x;
    stat.gamma_t = stat.gamma_ratio_t / h_t;

	cout << stat.gamma_s<< endl;
	cout << stat.gamma_t<< endl;
	stat.featureVector = new double*[stat.n];
	for (int i = 0; i < stat.n; i++)
		stat.featureVector[i] = new double[stat.dim+1];

	// file >> temp_string; file >> temp_string; file >> temp_string; file >> temp_string;
	for (int i = 0; i < stat.n; i++)
		for (int d = 0; d < stat.dim+1; d++)
			file >> stat.featureVector[i][d];


    cout << stat.featureVector[0][0]<< endl;
    cout << stat.featureVector[0][1]<< endl;
    cout << stat.featureVector[0][2]<< endl;
	cout << stat.featureVector[0][3]<< endl;
...
void init_visual(int argc, char**argv, statistics& stat)
{
	...
	if (stat.specify_region == false)
		updateRegion(stat); //Use the default region
...
void save_result_to_file_KDV(statistics& stat)
{
	fstream outputFile;
	outputFile.open(stat.outputFileName, ios::in | ios::out | ios::trunc);
	...
	for (int x = 0; x < stat.n_x; x++)
		for (int y = 0; y < stat.n_y; y++)
				outputFile << stat.out_tensor[x][y][0] << endl;
...
//    std::cout << "h_t = " << h_t << std::endl;
    ...
    stat.gamma_s = stat.gamma_ratio_s / h_x;
    stat.gamma_t = stat.gamma_ratio_t / h_t;
    std::cout << "gamma_s = " << stat.gamma_s << std::endl;
    std::cout << "gamma_t = " << stat.gamma_t << std::endl;
}